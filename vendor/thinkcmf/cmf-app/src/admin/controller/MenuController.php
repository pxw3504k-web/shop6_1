...
class MenuController extends AdminBaseController
{
    ...
    public function index()
    {
        ...
        foreach ($result as $key => $value) {
            ...
            $result[$key]['str_manage']     = '<a class="layui-btn layui-btn-primary layui-border-blue layui-btn-xs" href="' . url("Menu/add", ["parent_id" => $value['id'], "menu_id" => $this->request->param("menu_id")]) . '">' . lang('ADD_SUB_MENU') . '</a> 
                                               <a class="layui-btn layui-btn-primary layui-border-green layui-btn-xs" href="' . url("Menu/edit", ["id" => $value['id'], "menu_id" => $this->request->param("menu_id")]) . '">' . lang('EDIT') . '</a>  
                                               <a class="layui-btn layui-btn-primary layui-border-red layui-btn-xs js-ajax-delete" href="' . url("Menu/delete", ["id" => $value['id'], "menu_id" => $this->request->param("menu_id")]) . '">' . lang('DELETE') . '</a> ';
            //            $result[$key]['status']         = $value['status'] ? '<span class="layui-btn layui-btn-primary layui-border-green layui-btn-xs">' . lang('DISPLAY') . '</span>' : '<span class="layui-btn layui-btn-primary layui-border-orange layui-btn-xs">' . lang('HIDDEN') . '</span>';

            $checked = '';
            $id      = $value['id'];
            if ($value['status'] == 1) $checked = ' checked';
            $status                 = '<div class="layui-form">';
            $status                 .= '<input ' . $checked . ' type="checkbox" lay-skin="switch" lay-text="显示|隐藏" lay-filter="status" name="status" value="' . $id . '"';
        ...
        $str      = "<tr id='node-\$id' \$parent_id_node style='\$style'>
                        <td style='padding-left:20px;'><input name='list_orders[\$id]' type='text' size='3' value='\$list_order' class='input input-order' autocomplete='off'></td>
                        <td>\$id</td>
                        <td>\$spacer\$name</td>
                        <td>\$app</td>
                        <td>\$status</td>
    ...
    public function editPost()
    {
        ...
        if ($this->request->isPost()) {
            ...
            if ($result !== true) {
                ...
                AdminMenuModel::strict(false)->field(true)->update($params);
                $app          = $this->request->param("app");
                $controller   = $this->request->param("controller");
                $action       = $this->request->param("action");
                $param        = $this->request->param("param");
    ...
    public function delete()
    {
        if ($this->request->isPost()) {
            ...
            //            }
            if (AdminMenuModel::where('id', '=', $id)->update(['delete_time' => time()]) !== false) {
                $this->success("删除菜单成功！");
            } else {
                $this->error("删除失败！");
            }
    ...
    public function batch_post()
    {
        ...
        Db::name('admin_menu')->where('id', '=', $id)->update([
            'status' => $status,
        ]);
    ...
    public function listOrder()
    {
        ...
        if ($this->request->isPost()) {
            $modelName = 'admin_menu';
            $pk        = Db::name($modelName)->getPk(); //获取主键名称
            $ids       = $this->request->post("list_orders/a");
            if (!empty($ids)) {
                foreach ($ids as $key => $r) {
                    if ($r == '-d') {
                        Db::name($modelName)->where($pk, $key)->update(['delete_time' => time()]);//删除
                    } else {
                        $data['list_order'] = $r;
