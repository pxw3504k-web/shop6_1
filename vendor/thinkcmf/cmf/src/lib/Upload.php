...
//            die('');//分片没上传完
        ...
        $upload_type = cmf_get_option('storage');
        if (!empty($upload_type['type'])) {
            $alioss_config = cmf_get_plugin_config($upload_type['type']);
            $base_dir = !empty($alioss_config) ? $alioss_config['dir'] . '/' : '';

            $fileSaveName = $base_dir . (empty($app) ? '' : $app . '/') . $strDate . '/' . md5(uniqid()) . "." . $strFileExtension;
        } else {
