...
(129, 6, 1, 0, 10000, 'user', 'AdminUserAction', 'index', '', '用户操作管理', '', '用户操作管理'),
...
(133, 6, 1, 1, 0, 'admin', 'AppInfo', 'index', '', '应用设置', '', '应用设置');
...
(133, 1, 'admin', 'admin_url', 'admin/AppInfo/index', '', '应用设置', '');
...
(
    ...
    `id_card_image` text COMMENT '身份证照片',
    `is_top`        tinyint(2) DEFAULT '0' COMMENT '1置顶',
    `status`        tinyint(2) DEFAULT '0' COMMENT '1通过',
...
(
    ...
    `is_default`  tinyint(2) DEFAULT '0' COMMENT '1默认',
    PRIMARY KEY (`id`)
) ENGINE=InnoDB AUTO_INCREMENT=2 DEFAULT CHARSET=utf8mb4;

/*Data for the table `cmf_shop_address` */

insert into `cmf_shop_address`(`id`, `user_id`, `province`, `city`, `county`, `address`, `create_time`, `update_time`,
                               `status`, `username`, `phone`, `is_default`)
values (1, 1, '省', '市', '区', '地址', 1649127128, 1649127128, 1, '姓名', '17633940000', 1);

/*Table structure for table `cmf_shop_comment` */
...
(
    ...
    `content`         text,
    `thumb`           text,
    `is_show`         tinyint(2) DEFAULT '0' COMMENT '1显示',
    `create_time`     int(11) DEFAULT NULL,
    PRIMARY KEY (`id`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4;

/*Data for the table `cmf_shop_comment` */

/*Table structure for table `cmf_shop_coupon` */
...
(
    `id`           int(11) NOT NULL AUTO_INCREMENT,
    `type`         tinyint(3) DEFAULT '1' COMMENT '1通用 2餐饮',
    `name`         varchar(32) CHARACTER SET utf8 NOT NULL COMMENT '优惠券名称',
    `amount`       float                          NOT NULL COMMENT '面额',
    `full_amount`  float                          NOT NULL COMMENT '满额',
    `count`        int(11) NOT NULL COMMENT '数量',
    `select_count` int(11) NOT NULL DEFAULT '0' COMMENT '领取数量',
    `limit`        int(11) DEFAULT '1' COMMENT '限领取数',
    `status`       tinyint(4) DEFAULT '2' COMMENT '0删除 1显示 2隐藏',
...
/*Data for the table `cmf_shop_coupon` */

/*Table structure for table `cmf_shop_coupon_user` */
...
(
    ...
    `status`      tinyint(2) NOT NULL DEFAULT '1' COMMENT '0删除 1正常',
    `used`        tinyint(4) DEFAULT '0' COMMENT '1已使用 2已过期',
    `start_time`  int(11) DEFAULT NULL,
    `end_time`    int(11) DEFAULT NULL,
    `amount`      float        DEFAULT '0',
    `full_amount` float        DEFAULT '0',
    `name`        varchar(32)  DEFAULT NULL,
    `code`        varchar(32)  DEFAULT NULL,
    `qr_image`    varchar(100) DEFAULT NULL,
    `order_num`   varchar(32)  DEFAULT NULL,
    `is_send`     tinyint(4) DEFAULT '0' COMMENT '1赠送的',
    `type`        tinyint(4) DEFAULT NULL,
    PRIMARY KEY (`id`),
    KEY           `code` (`code`)
) ENGINE=InnoDB AUTO_INCREMENT=52 DEFAULT CHARSET=utf8;

/*Data for the table `cmf_shop_coupon_user` */

/*Table structure for table `cmf_shop_goods` */
...
/*Data for the table `cmf_shop_goods` */

/*Table structure for table `cmf_shop_goods_sku` */
...
/*Data for the table `cmf_shop_goods_sku` */

/*Table structure for table `cmf_shop_order` */
...
(
    `id`            int(11) NOT NULL AUTO_INCREMENT,
    `type`          tinyint(2) DEFAULT '1' COMMENT '订单类型',
    `shop_id`       int(11) DEFAULT NULL COMMENT '门店id',
    ...
    `county`        varchar(15)   DEFAULT NULL COMMENT '区',
    ...
    `pay_type`      tinyint(2) DEFAULT '1' COMMENT '1微信支付 2余额支付',
    `order_num`     varchar(40)   DEFAULT NULL COMMENT '订单号',
    `remark`        varchar(300)  DEFAULT '' COMMENT '备注',
    `status`        tinyint(2) DEFAULT '1' COMMENT '1待付款 2已付款 4已发货 6已收货 8已完成 10未付款取消 12退款申请 14退款不通过 16退款通过',
    ...
    `delete_time`   int(11) DEFAULT '0',
    PRIMARY KEY (`id`),
    KEY             `dindex` (`user_id`,`order_num`)
) ENGINE=InnoDB AUTO_INCREMENT=5 DEFAULT CHARSET=utf8mb4;

/*Data for the table `cmf_shop_order` */

/*Table structure for table `cmf_shop_order_detail` */
...
(
    ...
    `order_num`   varchar(40)   DEFAULT '0',
    `create_time` int(11) DEFAULT NULL,
    PRIMARY KEY (`id`)
) ENGINE=InnoDB AUTO_INCREMENT=7 DEFAULT CHARSET=utf8mb4;

/*Data for the table `cmf_shop_order_detail` */

/*Table structure for table `cmf_shop_order_refund` */
...
(
    ...
    `order_num`   varchar(40)   DEFAULT NULL COMMENT '退款订单号',
    `amount`      decimal(8, 2) DEFAULT NULL COMMENT '退款金额',
    `status`      tinyint(2) DEFAULT '0' COMMENT '1通过2驳回4已取消',
    `refund_num`  varchar(40)   DEFAULT NULL COMMENT '退款订单号',
