...
class Qrcode extends BasicWeChat
{
     ...
     * @param string $env_version 要打开的小程序版本。正式版为 release，体验版为 trial，开发版为 develop
     * @param integer $width 二维码的宽度
     * @param bool $auto_color 自动配置线条颜色，如果颜色依然是黑色，则说明不建议配置主色调
     * @param array $line_color auto_color 为 false 时生效
     * @param boolean $is_hyaline 是否需要透明底色
     ...
     * @throws \WeChat\Exceptions\LocalCacheException
     */
    public function createMiniScene($scene, $page, $env_version = 'release', $width = 430, $auto_color = false, $line_color = ["r" => "0", "g" => "0", "b" => "0"], $is_hyaline = false, $outType = null)
    {
        $check_path = false;//验证路径
        $is_hyaline = true;//是否需要透明底色
        $url  = 'https://api.weixin.qq.com/wxa/getwxacodeunlimit?access_token=ACCESS_TOKEN';
        $data = ['scene' => $scene, 'width' => $width, 'auto_color' => $auto_color, 'page' => $page, 'env_version' => $env_version, 'check_path' => $check_path, 'line_color' => $line_color, 'is_hyaline' => $is_hyaline];
        $this->registerApi($url, __FUNCTION__, func_get_args());
        $result = Tools::post($url, Tools::arr2json($data));
        if (is_array($json = json_decode($result, true))) {
            if (!$this->isTry && isset($json['errcode']) && in_array($json['errcode'], ['40014', '40001', '41001', '42001'])) {
