...
class ShopExpLogInit extends Base
{
    ...
    public function common_item($item = [], $params = [])
    {
        ...
        if ($this->InterfaceType == 'api') {
            /** api处理文件 **/
            if ($item['images']) $item['images'] = $this->getImagesUrl($item['images']);//图片
            ...
            /** admin处理文件 **/
            if ($item['images']) $item['images'] = $this->getParams($item['images']);//图片
    ...
    public function api_edit_post($params = [], $where = [])
    {
        ...
        /** 接口提交,处理数据 **/
        if ($params['images']) $params['images'] = $this->setParams($params['images']);//图片
    ...
    public function edit_post($params, $where = [])
    {
        ...
        /** 公共提交,处理数据 **/
        $order_info            = $ShopOrderModel->where('order_num', '=', $params['order_num'])->find();
        $params['exp_num']     = $order_info['exp_num'];
        $params['exp_name']    = $order_info['exp_name'];
        $params['exp_phone']   = $order_info['exp_phone'];
        $params['exp_car_num'] = $order_info['exp_car_num'];


        if (!empty($where)) {
            //传入where条件,根据条件更新数据
            $params["update_time"] = time();
            $result                = $ShopExpLogModel->where($where)->strict(false)->update($params);
            //if ($result) $result = $item["id"];
        } elseif (!empty($params["id"])) {
            //如传入id,根据id编辑数据
            $params["update_time"] = time();
            $result                = $ShopExpLogModel->where("id", "=", $params["id"])->strict(false)->update($params);
            //if($result) $result = $item["id"];
        } else {
            //无更新条件则添加数据
            $params["create_time"] = time();
            $result                = $ShopExpLogModel->strict(false)->insert($params, true);
        }
    ...
    public function edit_post_two($params, $where = [])
    {
        ...
        if (!empty($where)) {
            ...
            //无更新条件则添加数据
            $params["create_time"] = time();
            $result                = $ShopExpLogModel->strict(false)->insert($params);
        }
    ...
    public function delete_post($id, $type = 1, $params = [])
    {
        ...
        if ($type == 1) $result = $ShopExpLogModel->destroy($id);//软删除 数据表字段必须有delete_time
        if ($type == 2) $result = $ShopExpLogModel->destroy($id, true);//真实删除
    ...
    public function batch_post($id, $params = [])
    {
        ...
        $params["update_time"] = time();
        $result                = $ShopExpLogModel->where($where)->strict(false)->update($params);//修改状态
    ...
    public function list_order_post($list_order, $params = [])
    {
        ...
        foreach ($list_order as $k => $v) {
            $where   = [];
            $where[] = ["id", "=", $k];
            $result  = $ShopExpLogModel->where($where)->strict(false)->update(["list_order" => $v, "update_time" => time()]);//排序
        }
