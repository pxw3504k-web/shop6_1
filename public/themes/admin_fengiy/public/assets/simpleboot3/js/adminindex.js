...
function calcTaskContentWidth() {
    ...
    if (($(document).width() - 260 - tabwidth - 30 * 2) < width) {
        $("#task-content").width($(document).width() - 260 - tabwidth - 30 * 2);
...
$("#hamburger-container").click(function () {
	...
	var left = parseInt(sidebar.replace("px", ""));
	if(!left){
		$(this).html('<i class="fa fa-indent"></i>');
		$("#sidebar").animate({"left": "-210px"}, 300, 'swing');
		$(".main-content").animate({"margin-left": "0px"}, 300, 'swing');
		return
	}else{
		$(this).html('<i class="fa fa-outdent"></i>');
		$("#sidebar").animate({"left": "0px"}, 300, 'swing');
		$(".main-content").animate({"margin-left": "210px"}, 300, 'swing');
		return
	}
})