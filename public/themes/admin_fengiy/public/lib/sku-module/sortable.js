...
(function(global, factory) {
				...
				animationStates.forEach(function(state) {
					...
					if (target.thisAnimationDuration) {
						...
						if (isRectEqual(prevFromRect, toRect) && !isRectEqual(fromRect, toRect) &&
							...
							.left) === (fromRect.top - toRect.top) / (fromRect.left - toRect.left)
							) {
	...
	var PluginManager = {
		...
		pluginEvent: function pluginEvent(eventName, sortable, evt) {
			...
			return; // Fire global events if it exists in this sortable
				...
				if (sortable.options[plugin.pluginName] && sortable[plugin.pluginName][
						eventName]) {
		...
		getEventProperties: function getEventProperties(name, sortable) {
			...
			plugins.forEach(function(plugin) {
				...
				_extends(eventProperties, plugin.eventProperties.call(sortable[plugin
					.pluginName], name));
			});
		...
		modifyOption: function modifyOption(sortable, name, value) {
			...
			return; // If static option listener exists for this option, call in the context of the Sortable's instance of this plugin
				...
				if (plugin.optionListeners && typeof plugin.optionListeners[name] ===
					'function') {
					modifiedValue = plugin.optionListeners[name].call(sortable[plugin
						.pluginName], value);
				}
			});
	...
	function dispatchEvent(_ref) {
		...
		var allEventProperties = _objectSpread({}, extraEventProperties, PluginManager.getEventProperties(name,
			sortable));
	...
	Sortable.prototype =
		/** @lends Sortable.prototype */
		{
			...
			_onTapStart: function _onTapStart(
				...
				el); // Don't trigger start event when an element is been dragged, otherwise the evt.oldindex always wrong when set option.group.
			...
			_onDragOver: function _onDragOver(
				...
				function dragOverEvent(name, extra) {
					pluginEvent(name, _this, _objectSpread({
						...
						onMove: function onMove(target, after) {
							return _onMove(rootEl, el, dragEl, dragRect, target, getRect(
								target), evt, after);
						},
						changed: changed
					}, extra));
				...
				if (activeSortable && !options.disabled && (isOwner ? canSort || (revert = !rootEl.contains(
							dragEl)) // Reverting item into the original list
						:
						putSortable === this || (this.lastPutMode = activeGroup.checkPull(this,
							activeSortable, dragEl, evt)) && group.checkPut(this, activeSortable, dragEl,
							evt))) {
					...
					if (!elLastChild || _ghostIsLast(evt, vertical, this) && !elLastChild.animated) {
						...
						if (_onMove(rootEl, el, dragEl, dragRect, target, targetRect, evt, !!target) !==
							false) {
							capture();
						...
						if (direction !== 0) {
							...
							} while (sibling && (css(sibling, 'display') === 'none' || sibling ===
								ghostEl));
	...
	function AutoScrollPlugin() {
		...
		AutoScroll.prototype = {
			...
			nulling: function nulling() {
				touchEvt$1 = scrollRootEl = scrollEl = scrolling = pointerElemChangedInterval =
					lastAutoScrollX = lastAutoScrollY = null;
			...
			_handleAutoScroll: function _handleAutoScroll(evt, fallback) {
				...
				if (fallback || Edge || IE11OrLess || Safari) {
					autoScroll(evt, this.options, elem,
					fallback); // Listener for pointer element change

					var ogElemScroller = getParentAutoScrollElement(elem, true);

					if (scrolling && (!pointerElemChangedInterval || x !== lastAutoScrollX || y !==
							lastAutoScrollY)) {
						pointerElemChangedInterval &&
					clearPointerElemChangedInterval(); // Detect for pointer elem change, emulating native DnD behaviour

						pointerElemChangedInterval = setInterval(function() {
							var newElem = getParentAutoScrollElement(document.elementFromPoint(
								x, y), true);
					...
					if (!this.options.bubbleScroll || getParentAutoScrollElement(elem, true) ===
						getWindowScrollingElement()) {
						clearAutoScrolls();
						return;
					}
	...
	var autoScroll = throttle(function(evt, options, rootEl, isFallback) {
		...
		do {
			...
			if (el === winScroller) {
				canScrollX = width < scrollWidth && (elCSS.overflowX === 'auto' || elCSS.overflowX ===
					'scroll' || elCSS.overflowX === 'visible');
				canScrollY = height < scrollHeight && (elCSS.overflowY === 'auto' || elCSS.overflowY ===
					'scroll' || elCSS.overflowY === 'visible');
			} else {
				canScrollX = width < scrollWidth && (elCSS.overflowX === 'auto' || elCSS.overflowX ===
					'scroll');
				canScrollY = height < scrollHeight && (elCSS.overflowY === 'auto' || elCSS.overflowY ===
			...
			var vx = canScrollX && (Math.abs(right - x) <= sens && scrollPosX + width < scrollWidth) - (
				Math.abs(left - x) <= sens && !!scrollPosX);
			var vy = canScrollY && (Math.abs(bottom - y) <= sens && scrollPosY + height <
				scrollHeight) - (Math.abs(top - y) <= sens && !!scrollPosY);
			...
			if (autoScrolls[layersOut].vx != vx || autoScrolls[layersOut].vy != vy || autoScrolls[
					layersOut].el !== el) {
				...
				if (vx != 0 || vy != 0) {
					...
					autoScrolls[layersOut].pid = setInterval(function() {
						...
						if (isFallback && this.layer === 0) {
							Sortable.active._onTouchMove(
							touchEvt$1); // To move ghost if it is positioned absolutely

						}

						var scrollOffsetY = autoScrolls[this.layer].vy ? autoScrolls[this.layer]
							.vy * speed : 0;
						var scrollOffsetX = autoScrolls[this.layer].vx ? autoScrolls[this.layer]
							.vx * speed : 0;

						if (typeof scrollCustomFn === 'function') {
							if (scrollCustomFn.call(Sortable.dragged.parentNode[expando],
									scrollOffsetX, scrollOffsetY, evt, touchEvt$1, autoScrolls[
										this.layer].el) !== 'continue') {
								return;
							}
						}
			...
			layersOut++;
		} while (options.bubbleScroll && currentParent !== winScroller && (currentParent =
				getParentAutoScrollElement(currentParent, false)));

		scrolling =
		scrollThisInstance; // in case another function catches scrolling as false in between when it is not
	}, 30);
		...
		MultiDrag.prototype = {
			...
			setupClone: function setupClone(_ref2) {
				...
				for (var i = 0; i < multiDragElements.length; i++) {
					...
					multiDragElements[i] === dragEl$1 && toggleClass(multiDragClones[i], this.options
						.chosenClass, false);
				}
			...
			dragOverCompleted: function dragOverCompleted(_ref10) {
				...
				if (insertion) {
					...
					initialFolding =
					false; // If leaving sort:false root, or already folding - Fold to new location

					if (options.animation && multiDragElements.length > 1 && (folding || !isOwner && !
							activeSortable.options.sort && !putSortable)) {
						// Fold: Set all multi drag elements's rects to dragEl's rect when multi-drag elements are invisible
						var dragRectAbsolute = getRect(dragEl$1, false, true, true);
						multiDragElements.forEach(function(multiDragElement) {
							if (multiDragElement === dragEl$1) return;
							setRect(multiDragElement,
							dragRectAbsolute); // Move element(s) to end of parentEl so that it does not interfere with multi-drag clones insertion if they are inserted
					...
					if (!isOwner) {
						...
						if (multiDragElements.length > 1) {
							...
							activeSortable._showClone(
							sortable); // Unfold animation for clones if showing from hidden


							if (activeSortable.options.animation && !clonesHidden &&
								clonesHiddenBefore) {
								multiDragClones.forEach(function(clone) {
			...
			dragOverAnimationCapture: function dragOverAnimationCapture(_ref11) {
				...
				if (activeSortable.options.animation && !isOwner && activeSortable.multiDrag
					.isMultiDrag) {
			...
			drop: function drop(_ref12) {
				...
				if (!dragStarted) {
					...
					if (!~multiDragElements.indexOf(dragEl$1)) {
						...
						if (evt.shiftKey && lastMultiDragSelect && sortable.el.contains(
								lastMultiDragSelect)) {
				...
				if (dragStarted && this.isMultiDrag) {
					...
					if ((parentEl[expando].options.sort || parentEl !== rootEl) && multiDragElements
						.length > 1) {
						...
						if (!initialFolding) {
							if (options.animation) {
								...
								multiDragElements.forEach(function(multiDragElement) {
									...
									if (multiDragElement !== dragEl$1) {
										var rect = folding ? getRect(multiDragElement) :
											dragRect;
										multiDragElement.fromRect =
										rect; // Prepare unfold animation
							...
							multiDragElements.forEach(function(multiDragElement) {
								if (children[multiDragIndex]) {
									parentEl.insertBefore(multiDragElement, children[
										multiDragIndex]);
								} else {
									parentEl.appendChild(multiDragElement);
								}
			...
			return; // Only deselect if selection is in this sortable

				if (multiDragSortable !== this.sortable)
			return; // Only deselect if target is not item in this sortable
			...
			eventProperties: function eventProperties() {
				...
				multiDragElements.forEach(function(multiDragElement) {
					...
					if (folding && multiDragElement !== dragEl$1) {
						...
						newIndex = index(multiDragElement, ':not(.' + _this3.options
							.selectedClass + ')');
					} else {
	...
	function insertMultiDragElements(clonesInserted, rootEl) {
		multiDragElements.forEach(function(multiDragElement, i) {
			var target = rootEl.children[multiDragElement.sortableIndex + (clonesInserted ? Number(i) :
				0)];
